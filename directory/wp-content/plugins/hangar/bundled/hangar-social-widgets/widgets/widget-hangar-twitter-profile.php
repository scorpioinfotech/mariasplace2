...
/**
 * Hangar Twitter Profile Widget
 *
 * A bundled Hangar Twitter Profile widget.
 *
 * @package WordPress
 * @subpackage Hangar
 * @category Bundled
 ...
 * - enqueue_styles()
 */
class Hangar_Widget_TwitterProfile extends WP_Widget {
	...
	function __construct () {
		...
		$this->woo_widget_cssclass = 'widget_hangar_twitterprofile';
		$this->woo_widget_description = __( 'This is a Hangar bundled Twitter profile widget.', 'hangar' );
		$this->woo_widget_idbase = 'hangar_twitterprofile';
		$this->woo_widget_title = __('Hangar - Twitter Profile', 'hangar' );
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['display_friends_count'] == true && isset( $data->friends_count ) ) {
			$html .= '<p class="friends stat"><span class="number">' . $data->friends_count . '</span> <span class="stat-label">' . __( 'friends', 'hangar' ) . '</span></p>' . "\n";
		}

		if ( $instance['display_follower_count'] == true && isset( $data->followers_count ) ) {
			$html .= '<p class="followers stat"><span class="number">' . $data->followers_count . '</span> <span class="stat-label">' . __( 'followers', 'hangar' ) . '</span></p>' . "\n";
		}

		if ( $instance['display_status_count'] == true && isset( $data->statuses_count ) ) {
			$html .= '<p class="statuses stat"><span class="number">' . $data->statuses_count . '</span> <span class="stat-label">' . __( 'tweets', 'hangar' ) . '</span></p>' . "\n";
		}
		...
		if ( $instance['display_tweeting_since'] == true && isset( $data->statuses_count ) ) {
			$html .= '<p class="tweeting-since">' . __( 'Tweeting since', 'hangar' ) . ' <span class="date">' . date( get_option( 'date_format' ), strtotime( $data->created_at ) ) . '</span>' . '</p>' . "\n";
		}

		if ( $instance['include_follow_link'] != false ) {
			$html .= '<p class="follow-link"><a href="' . esc_url( 'http://twitter.com/' . urlencode( $instance['twitter_handle'] ) ) . '">' . sprintf( __( 'Follow %s on Twitter', 'hangar' ), $instance['twitter_handle'] ) . '</a></p>' . "\n";
		}
   ...
   function form ( $instance ) {
		...
		$defaults = array(
						'title' => __( 'Twitter Profile', 'hangar' ), 
						'twitter_handle' => '', 
						'display_avatar' => 1, 
						'display_name' => 1, 
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_handle' ); ?>"><?php _e( 'Twitter Username (required):', 'hangar' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'avatar_alignment' ); ?>"><?php _e( 'Avatar Alignment:', 'hangar' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'avatar_alignment' ); ?>" class="widefat" id="<?php echo $this->get_field_id( 'avatar_alignment' ); ?>">
				<option value="left"<?php selected( $instance['avatar_alignment'], 'left' ); ?>><?php _e( 'Left', 'hangar' ); ?></option>
				<option value="centre"<?php selected( $instance['avatar_alignment'], 'centre' ); ?>><?php _e( 'Centre', 'hangar' ); ?></option>
				<option value="right"<?php selected( $instance['avatar_alignment'], 'right' ); ?>><?php _e( 'Right', 'hangar' ); ?></option>         
			</select>
		</p>
...
<?php
	...
	private function get_checkbox_settings () {
		return array(
					'display_avatar' => __( 'Display Avatar', 'hangar' ), 
					'display_name' => __( 'Display Name', 'hangar' ), 
					'display_screen_name' => __( 'Display Screen Name', 'hangar' ), 
					'display_description' => __( 'Display Description', 'hangar' ), 
					'display_location' => __( 'Display Location', 'hangar' ), 
					'display_status_count' => __( 'Display Tweet Count', 'hangar' ), 
					'display_follower_count' => __( 'Display Followers Count', 'hangar' ), 
					'display_friends_count' => __( 'Display Friends Count', 'hangar' ), 
					'display_tweeting_since' => __( 'Display "Tweeting Since"', 'hangar' ), 
					'include_follow_link' => __( 'Include "Follow" Link', 'hangar' )
					);
