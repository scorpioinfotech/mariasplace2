...
/**
 * Hangar Instagram Widget
 *
 * A bundled Hangar Instagram stream widget.
 *
 * @package WordPress
 * @subpackage Hangar
 * @category Bundled
 ...
 */
class Hangar_Widget_Instagram extends WP_Widget {
	...
	public function __construct () {
		...
		$this->woo_widget_cssclass = 'widget_hangar_instagram';
		$this->woo_widget_description = __( 'This is a Hangar bundled Instagram stream widget.', 'hangar' );
		$this->woo_widget_idbase = 'hangar_instagram';
		$this->woo_widget_title = __('Hangar - Instagram', 'hangar' );
		...
		/* Setup the assets URL in relation to Hangar. */
		$this->assets_url = trailingslashit( $hangar->base->components_url . 'hangar-social-widgets/assets' );
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title (optional):', 'hangar' ); ?></label>
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Username (required):', 'hangar' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'password' ); ?>"><?php _e( 'Password (required):', 'hangar' ); ?></label>
...
?>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Limit (default: 5):', 'hangar' ); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'float' ); ?>"><?php _e( 'Float Images:', 'hangar' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'float' ); ?>" class="widefat" id="<?php echo $this->get_field_id( 'float' ); ?>">
			<option value="none"<?php selected( $instance['float'], 'none' ); ?>><?php _e( 'None', 'hangar' ); ?></option>
			<option value="left"<?php selected( $instance['float'], 'left' ); ?>><?php _e( 'Left', 'hangar' ); ?></option>
			<option value="right"<?php selected( $instance['float'], 'right' ); ?>><?php _e( 'Right', 'hangar' ); ?></option>         
		</select>
	</p>
	<!-- Widget Image Size: Select Input -->
	<p>
		<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e( 'Image Size:', 'hangar' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'image_size' ); ?>" class="widefat" id="<?php echo $this->get_field_id( 'image_size' ); ?>">
			<option value="thumbnail"<?php selected( $instance['image_size'], 'thumbnail' ); ?>><?php _e( 'Thumbnail (150x150)', 'hangar' ); ?></option>
			<option value="low_resolution"<?php selected( $instance['image_size'], 'low_resolution' ); ?>><?php _e( 'Low Resolution (306x306)', 'hangar' ); ?></option>
			<option value="standard_resolution"<?php selected( $instance['image_size'], 'standard_resolution' ); ?>><?php _e( 'Standard Resolution (612x612)', 'hangar' ); ?></option>         
		</select>
	</p>
	<!-- Widget Custom Image Size: Text Input -->
	<p>
		<label for="<?php echo $this->get_field_id( 'custom_image_size' ); ?>"><?php _e( 'Custom Image Size (max: 612):', 'hangar' ); ?></label>
		<input type="text" name="<?php echo $this->get_field_name( 'custom_image_size' ); ?>"  value="<?php echo $instance['custom_image_size']; ?>" class="widefat" style="width: 30%;" maxlength="3" id="<?php echo $this->get_field_id( 'custom_image_size' ); ?>" /> <?php _e( 'px', 'hangar' ); ?>
	</p>
	<?php foreach ( $checkboxes as $k => $v ) { ?>
	<!-- Widget <?php echo $v; ?>: Checkbox Input -->
	<p>
	...
	<?php
		if ( $k == 'enable_thickbox' ) {
			echo '<p><small>(' . __( 'Requires the "Link to full size image" option', 'hangar' ) . ')</small></p>' . "\n";
		}
	?>
	<?php } ?>
	<hr />
	<p><?php printf( __( 'Currently logged in as %s.', 'hangar' ), '<strong>' . $instance['username'] . '</strong>' ); ?></p>
	...
	<p>
    	...
    	<label for="<?php echo $this->get_field_id( 'logout' ); ?>"><?php _e( 'Logout?', 'hangar' ); ?></label>
	</p>
<?php
	} // End IF Statement
	...
	public function enqueue_styles () {
		wp_register_style( 'hangar-social-widgets', $this->assets_url . 'css/style.css' );
		wp_enqueue_style( 'hangar-social-widgets' );
	...
	private function prepare_photos_html ( $data, $instance ) {
		...
		if ( is_object( $data ) && isset( $data->data ) && is_array( $data->data ) && ( count( $data->data ) > 0 ) ) {
			...
			foreach ( $data->data as $k => $v ) {
				...
				if ( $caption == '' ) {
					$caption = sprintf( __( 'Instagram by %s', 'hangar' ), $v->user->full_name );
				}
	...
	private function get_checkbox_settings () {
		return array(
					'link_to_fullsize' => __( 'Link to full size image', 'hangar' ), 
					'enable_thickbox' => __( 'Enable thickbox', 'hangar' )
					);
