<?php
	...
	$fields = apply_filters("tdp_authnet_silent_post_fields", $fields);
	do_action("tdp_before_authnet_silent_post", $fields);
	...
	if($arb == true)
	{
		...
		$old_order_id = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE subscription_transaction_id = '" . esc_sql($fields['x_subscription_id']) . "' AND gateway = 'authorizenet' ORDER BY timestamp DESC LIMIT 1");
		...
		else
		{
			...
			$tdpemail = new TDpEmail();	
			$tdpemail->data = array("body"=>__("<p>A payment is being held for review within Authorize.net.</p><p>Payment Information From Authorize.net", "tdp") . ":<br />" . nl2br(var_export($fields, true)));
			$tdpemail->sendEmail(get_bloginfo("admin_email"));			
		}
	}	

	do_action("tdp_after_authnet_silent_post", $fields);
