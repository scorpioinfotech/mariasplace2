<?php
	class MemberOrder
	{
		...
		{			
			//setup the gateway			
			$this->setGateway(tdp_getOption("gateway"));
		...
		function getMemberOrderByID($id)
		{
			...
			$dbobj = $wpdb->get_row("SELECT *, UNIX_TIMESTAMP(timestamp) + " . ($gmt_offset * 3600) . "  as timestamp FROM $wpdb->tdp_membership_orders WHERE id = '$id' LIMIT 1");
		...
		function setGateway($gateway = NULL)
		{
			...
			//which one to load?
			$classname = "TDpGateway";	//default test gateway
			...
			{
				$error = new WP_Error("TDp1001", "Could not locate the gateway class file with class name = " . $classname . ".");
		...
		function getLastMemberOrder($user_id = NULL, $status = 'success')
		{
			...
			//build query
			$this->sqlQuery = "SELECT id FROM $wpdb->tdp_membership_orders WHERE user_id = '" . $user_id . "' ";
		...
		{
			global $wpdb;
			$id = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE code = '" . $code . "' LIMIT 1");
			if($id)
				return $this->getMemberOrderByID($id);
			else
				return false;
		...
		{
			global $wpdb;
			$id = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE payment_transaction_id = '" . esc_sql($payment_transaction_id) . "' LIMIT 1");
			if($id)
				return $this->getMemberOrderByID($id);
			else
				return false;
		...
		*/
		function getLastMemberOrderBySubscriptionTransactionID($subscription_transaction_id)
		{
			...
			global $wpdb;
			$id = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE subscription_transaction_id = '" . esc_sql($subscription_transaction_id) . "' ORDER BY id DESC LIMIT 1");
		...
		{
			global $wpdb;
			$id = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE paypal_token = '" . $token . "' LIMIT 1");			
			if($id)
				return $this->getMemberOrderByID($id);
			else
				return false;
		...
		function getDiscountCode($force = false)
		{
			...
			global $wpdb;
			$this->discount_code = $wpdb->get_row("SELECT dc.* FROM $wpdb->tdp_discount_codes dc LEFT JOIN $wpdb->tdp_discount_codes_uses dcu ON dc.id = dcu.code_id WHERE dcu.order_id = '" . $this->id . "' LIMIT 1");
		...
		function getMembershipLevel($force = false)
		{
			...
			{
				$this->membership_level = $wpdb->get_row("SELECT l.id as level_id, l.name, l.description, l.allow_signups, l.expiration_number, l.expiration_period, mu.*, UNIX_TIMESTAMP(mu.startdate) as startdate, UNIX_TIMESTAMP(mu.enddate) as enddate, l.name, l.description, l.allow_signups FROM $wpdb->tdp_membership_levels l LEFT JOIN $wpdb->tdp_memberships_users mu ON l.id = mu.membership_id WHERE mu.status = 'active' AND l.id = '" . $this->membership_id . "' AND mu.user_id = '" . $this->user_id . "' LIMIT 1");
			...
			{
				$sqlQuery = "SELECT l.id, cl.*, l.name, l.description, l.allow_signups FROM $wpdb->tdp_discount_codes_levels cl LEFT JOIN $wpdb->tdp_membership_levels l ON cl.level_id = l.id LEFT JOIN $wpdb->tdp_discount_codes dc ON dc.id = cl.code_id WHERE dc.code = '" . $this->discount_code . "' AND cl.level_id = '" . $this->membership_id . "' LIMIT 1";			
			...
			{
				$this->membership_level = $wpdb->get_row("SELECT l.* FROM $wpdb->tdp_membership_levels l WHERE l.id = '" . $this->membership_id . "' LIMIT 1");			
			}
		...
		function getTaxForPrice($price)
		{
			...
			//filter
			$tax = apply_filters("tdp_tax", $tax, $values, $this);			
			return $tax;
		}
		...
		function updateTimestamp($year, $month, $day, $time = NULL)
		{
			...
			global $wpdb;
			$this->sqlQuery = "UPDATE $wpdb->tdp_membership_orders SET timestamp = '" . $date . "' WHERE id = '" . $this->id . "' LIMIT 1";
		...
		{			
			...
			if(empty($this->gateway))
				$this->gateway = tdp_getOption("gateway");				
			if(empty($this->gateway_environment))
				$this->gateway_environment = tdp_getOption("gateway_environment");
			...
			{
				//set up actions
				$before_action = "tdp_update_order";
				$after_action = "tdp_updated_order";
				//update
				$this->sqlQuery = "UPDATE $wpdb->tdp_membership_orders
									SET `code` = '" . $this->code . "',
			...
			{
				//set up actions
				$before_action = "tdp_add_order";
				$after_action = "tdp_added_order";
				//insert
				$this->sqlQuery = "INSERT INTO $wpdb->tdp_membership_orders  
		...
		function getRandomCode()
		{
			...
			while(empty($code))
			{
				...
				$code = apply_filters("tdp_random_code", $code, $this);	//filter				
				$check = $wpdb->get_var("SELECT id FROM $wpdb->tdp_membership_orders WHERE code = '$code' LIMIT 1");				
		...
		function updateStatus($newstatus)
		{
			...
			$this->sqlQuery = "UPDATE $wpdb->tdp_membership_orders SET status = '" . esc_sql($newstatus) . "' WHERE id = '" . $this->id . "' LIMIT 1";
			if($wpdb->query($this->sqlQuery) !== false)
				return true;
			else
				return false;
		...
		function deleteMe()
		{
			...
			global $wpdb;
			$this->sqlQuery = "DELETE FROM $wpdb->tdp_membership_orders WHERE id = '" . $this->id . "' LIMIT 1";
			if($wpdb->query($this->sqlQuery) !== false)
			{
				do_action("tdp_delete_order", $this->id, $this);
				return true;
			}
			else
				return false;
