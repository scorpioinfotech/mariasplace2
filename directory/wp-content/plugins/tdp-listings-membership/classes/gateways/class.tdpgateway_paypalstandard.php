<?php
	...
	class TDpGateway_paypalstandard
	{
		function TDpGateway_paypalstandard($gateway = NULL)
		{
		...
		{						
			...
			$environment = tdp_getOption("gateway_environment");
			if("sandbox" === $environment || "beta-sandbox" === $environment)
				$paypal_url ="https://www.sandbox.paypal.com/cgi-bin/webscr?business=" . urlencode(tdp_getOption("gateway_email"));
			else
				$paypal_url = "https://www.paypal.com/cgi-bin/webscr?business=" . urlencode(tdp_getOption("gateway_email"));
			...
			{				
				...
				$paypal_args = array( 
					...
					'shipping'      => '0',
					'no_note'       => '1', 
					'currency_code' => $tdp_currency, 
					'item_number'   => $order->code, 
					'charset'       => get_bloginfo( 'charset' ), 				
					'rm'            => '2', 
					'return'        => tdp_url("confirmation", "?level=" . $order->membership_level->id),
				...
				*/
				...
				else
				{
					...
					$adjusted_psd = apply_filters("tdp_profile_start_date", $psd, $order);
					if($psd != $adjusted_psd)
					{
			...
			$additional_parameters = apply_filters("tdp_paypal_express_return_url_parameters", array());									
			...
			$nvpStr = apply_filters("tdp_paypal_standard_nvpstr", $nvpStr, $order);
		...
		function PPHttpPost($methodName_, $nvpStr_) {
			...
			$API_UserName = tdp_getOption("apiusername");
			$API_Password = tdp_getOption("apipassword");
			$API_Signature = tdp_getOption("apisignature");
