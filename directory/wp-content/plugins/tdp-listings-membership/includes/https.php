...
function tdp_https_filter($s)
{
	global $besecure;
	$besecure = apply_filters('tdp_besecure', $besecure);
...
add_filter('status_header', 'tdp_status_filter');
add_filter('bloginfo_url', 'tdp_https_filter');
add_filter('wp_list_pages', 'tdp_https_filter');
add_filter('option_home', 'tdp_https_filter');
add_filter('option_siteurl', 'tdp_https_filter');
add_filter('logout_url', 'tdp_https_filter');
add_filter('login_url', 'tdp_https_filter');
add_filter('home_url', 'tdp_https_filter');

//this function sets the besecure global which may be used in early code
/*
function tdp_besecure_set()
{	
	global $besecure;		
...
function tdp_besecure()
{
	...
	if(!$besecure && force_ssl_admin() && (is_admin() || tdp_is_login_page()))
		$besecure = true;		
		
	//if forcing ssl on login, be secure on the login page
	if(!$besecure && force_ssl_login() && tdp_is_login_page())
		$besecure = true;			
...
add_action('wp', 'tdp_besecure', 2);
add_action('login_init', 'tdp_besecure', 2);

//If the site URL starts with https:, then force SSL/besecure to true. (Added 1.5.2)
function tdp_check_site_url_for_https($besecure)
{	
	global $wpdb, $tdp_siteurl;

	//need to get this from the database because we filter get_option
	if(empty($tdp_siteurl))
		$tdp_siteurl = $wpdb->get_var("SELECT option_value FROM $wpdb->options WHERE option_name = 'siteurl' LIMIT 1");		
	
	//entire site is over https?
	if(strpos($tdp_siteurl, "https:") !== false)
		$besecure = true;
	
	return $besecure;
}
add_filter("tdp_besecure", "tdp_check_site_url_for_https");

//capturing case where a user links to https admin without admin over https
function tdp_admin_https_handler()
{
...
add_action('init', 'tdp_admin_https_handler');

/*
	This code is for the "nuke" option to make URLs secure on secure pages.
*/
function tdp_NuclearHTTPS()
{
	//did they choose the option?
	$nuking = tdp_getOption("nuclear_HTTPS");
	if(!empty($nuking))
	{
		ob_start("tdp_replaceURLsInBuffer");
	}
}
add_action("init", "tdp_NuclearHTTPS");

function tdp_replaceURLsInBuffer($buffer)
{
	global $besecure;
	...
	if($besecure)
	{
		...
		*/
		$buffer = preg_replace("/http\:\/\/([a-zA-Z0-9\.\-]*" . str_replace(".", "\.", TDP_DOMAIN) . ")/i", "https://$1", $buffer);		
	}
