<?php	
	...
	if(!function_exists("current_user_can") || (!current_user_can("manage_options") && !current_user_can("tdp_orders_csv")))
	{
	...
	{
		$sqlQuery = "SELECT SQL_CALC_FOUND_ROWS id FROM $wpdb->tdp_membership_orders ORDER BY timestamp DESC ";
	}
	...
	//any extra columns
	$extra_columns = apply_filters("tdp_orders_csv_extra_columns", array());
	...
	if($order_ids)
	{
		foreach($order_ids as $order_id)
		{
			...
			$sqlQuery = "SELECT c.id, c.code FROM $wpdb->tdp_discount_codes_uses cu LEFT JOIN $wpdb->tdp_discount_codes c ON cu.code_id = c.id WHERE cu.order_id = '" . $order_id . "' LIMIT 1";
			...
			if(!empty($default_columns))
			{
				...
				foreach($default_columns as $col)
				{
					...
					if(!empty($col[2]) && isset($$col[0]->$col[1]->$col[2]))
						$csvoutput .= tdp_enclose($$col[0]->$col[1]->$col[2]);	//output the value				
			...
			//timestamp
			$csvoutput .= "," . tdp_enclose(date(get_option("date_format"), $order->timestamp));
			...
			if(!empty($extra_columns))
			{
				foreach($extra_columns as $heading => $callback)
				{
					$csvoutput .= "," . tdp_enclose(call_user_func($callback, $order));
				}
			}
