<?php
/*
Plugin Name: BuddyPress Reported

Description: Gives users the ability to 'Report' content across your BuddyPress enabled site. Modified by @webdwall to show both like and unlike buttons simultaneously
		...
		if ( is_user_logged_in() && $activity_type !== 'activity_liked' && $activity_type !== 'activity_unliked' ) :
			...
			*/
			...
			<?php if (bp_like_is_liked( bp_get_activity_id(), 'activity' ) ) { ?>
					...
					<span class="icon-flagged"></span> Reported
			...
			<?php } ?>
			<?php if ((!bp_like_is_unliked( bp_get_activity_id(), 'activity' )) && (!bp_like_is_liked( bp_get_activity_id(), 'activity' )) ) { ?>
				<a href="#" class="like" id="like-activity-<?php bp_activity_id(); ?>" 
					title="<?php echo bp_like_get_text( 'like_this_item' ); ?>">
					<span class="icon-flag"></span>
					<?php //echo bp_like_get_text( 'like' ); //if ( $liked_count ) echo ' (' . $liked_count . ')'; ?></a>
			<?php } ?>
			<?php	echo"<span> Comment ".bp_activity_comment_count()."</span>"; ?>
			<?php
			/*
			if ( $users_who_like ): ?>
