...
class WooCommerce_Product_Vendors {
    ...
    /**
     * Handle WC settings for Product Vendors
     * @param  array $settings Current settings
     * @return array $settings Modified settings
     */
    // public function manage_settings( $settings ) {

    //     $settings[] = array( 'title' => __( 'Product Vendors', 'wc_product_vendors' ), 'type' => 'title', 'desc' => '', 'id' => 'product_vendors' );
    //     $settings[] = array(
    //                         'title'     => __( 'Default commission (%)', 'wc_product_vendors' ),
    //                         'desc'      => __( 'The default percentage of the sale price that will go to the product vendor. This can be changed on per vendor and per product.', 'wc_product_vendors' ),
    //                         'id'        => 'wc_product_vendors_global_commission',
    //                         'css'       => 'width:50px;',
    //                         'default'   => '0',
    //                         'type'      => 'text',
    //                         'desc_tip'  =>  true
    //                     );
    //     $settings[] = array( 'type' => 'sectionend', 'id' => 'product_vendors');
    ...
    public function vendor_details_page() {
        ...
        if( isset( $_POST[ 'update_vendor_' . $vendor->ID ] ) ) {
            if( ! wp_verify_nonce( $_POST['vendor_update_nonce'], 'vendor_update' ) ) {
                wp_die( __( 'Cheatin&#8217; uh?' ) );
            }

            // PayPal account email address
            $paypal_address = $_POST[ 'wc_product_vendors_paypal_address_' . $vendor->ID ];
            $vendor_data['paypal_email'] = $paypal_address;
            update_option( $this->token . '_' . $vendor->ID, $vendor_data );

            // Vendor description
            $args = array(
                'description' => $_POST[ 'wc_product_vendors_description_' . $vendor->ID ]
            );
            wp_update_term( $vendor->ID, $this->token, $args );

            do_action( 'product_vendors_details_fields_save', $vendor->ID, $_POST );
        ...
        echo '<div class="wrap" id="vendor_details">
                ...
                <form method="post" action="" enctype="multipart/form-data">
                    <input type="hidden" name="update_vendor_' . $vendor->ID . '" value="true" />
	...
	public function add_vendor_fields( $taxonomy ) {
        ...
        <div class="form-field">
            ...
            <span class="description"><?php _e( 'A list of users who can mange this vendors products and view the sales reports.', 'wc_product_vendors' ); ?></span>
        </div>
