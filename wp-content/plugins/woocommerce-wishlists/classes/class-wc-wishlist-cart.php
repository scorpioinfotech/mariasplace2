...
class WC_Wishlists_Cart {
	...
	function order_item_meta_2($item_id, $values) {
		if (function_exists('woocommerce_add_order_item_meta')) {
			...
			if (!empty($values['wishlist-data']) && isset($values['wishlist-data']['list']) && isset($values['wishlist-data']['item']) && isset($values['wishlist-data']['customer'])) {
				...
				woocommerce_add_order_item_meta($item_id, $list_data['name'], $name);

				woocommerce_add_order_item_meta($item_id, '_wishlist_id', $wishlist_id);
				woocommerce_add_order_item_meta($item_id, '_wishlist_item_id', $wishlist_item_id);
				woocommerce_add_order_item_meta($item_id, '_wishlist_customer_key', $wishlist_customer_key);
			}
		}
	}
	...
	public function on_order_completed($order_id, $current_status, $new_status) {
		...
		$auto_remove_on_status = WC_Wishlists_Settings::get_setting('wc_wishlist_processing_autoremove_status', 'completed');

		if ($auto_remove_items && $new_status == $auto_remove_on_status) {
			...
			if ($items) {
				foreach ($items as $item) {
					if (isset($item['wishlist_id']) && isset($item['wishlist_item_id']) && isset($item['wishlist_customer_key'])) {
						...
						if ($wl_owner == $item['wishlist_customer_key'] || $wl_owner == $order->customer_user) {
