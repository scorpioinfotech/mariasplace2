...
class BP_Reshare extends BP_Component {
	...
	public function setup_activity_tab() {
		if ( buddyreshare_get_total_reshares_count( bp_loggedin_user_id() ) ) : ?>
			...
			<li id="activity-reshares"><a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/'. buddyreshare_get_component_slug() .'/'; ?>" title="<?php _e( 'Activities i shared.', 'bp-reshare' ); ?>"><?php printf( __( '%s <span>%s</span>', 'bp-reshare' ), __( 'My Shares', 'bp-reshares' ), buddyreshare_get_total_reshares_count( bp_loggedin_user_id() ) ); ?></a></li>
	...
	public function register_activity_actions() {
		...
		bp_activity_set_action( $this->id, 'reshare_update',        __( 'Shares', 'bp-reshare' ) );

		if( !is_admin() )
			bp_activity_set_action( $this->id, 'activity_mostreshared', __( 'Most Shared', 'bp-reshare' ) );
