...
function bp_activity_bump_comment_posted( $comment_id, $params ) {
	...
	if ( bp_activity_bump_denied_type_check( $activity_parent->type ) ) {
		return;
	}
	if ( !bp_activity_bump_denied_user_check() ) {
		return;
	}
...
function bp_activity_bump_time_since( $content, $activity ) {
	...
	return apply_filters( 'bp_activity_bump_time_since', '<span class="time-since time-created">' . sprintf( __( ' %s', 'buddypress' ), bp_core_time_since( $bumpdate ) ) . '</span> &middot; ' . $content, $activity->date_recorded, $bumpdate, $content );
}
add_filter( 'bp_activity_time_since', 'bp_activity_bump_time_since', 1, 2 );

function bp_activity_bump_denied_type_check( $type ) {
...
function bp_activity_bump_denied_user_check() {
	global $bp;
