<?php
/*
Plugin Name: WooCommerce PayPal Digital Goods Gateway
Plugin URI: http://www.woothemes.com/products/paypal-digital-goods-gateway/
Description: Frictionless payments with PayPal. PayPal Digital Goods is the only one-click PayPal payment solution.
Version: 2.4.1
...
*/
/*
	Copyright 2013 Leonard's Ego Inc.  (email : freedoms@leonardsego.com)
 ...
 * Plugin updates
 */
woothemes_queue_update( plugin_basename( __FILE__ ), 'd8070e7f7ae63b088e58480f3640e9e2', '18636' );
...
function ppdg_admin_notices() {
	...
	if ( ! is_woocommerce_active() ) { ?>
		<div id="message" class="error">
			<p><?php printf( __( '%sThe PayPal Digital Goods extension for WooCommerce is inactive.%s The %sWooCommerce plugin%s must be active for the PayPal Digital Goods extension to work. Please %sinstall & activate WooCommerce%s', 'ppdg' ), '<strong>', '</strong>', '<a href="http://wordpress.org/extend/plugins/woocommerce/">', '</a>', '<a href="' . admin_url( 'plugins.php' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
		</div>
	<?php } elseif ( get_transient( 'ppdg-activated' ) == 'true' ) {
		...
		<div class="updated woocommerce-message wc-connect">
			<p>
				<?php printf( __( '%sPayPal Digital Goods Installed%s &#8211; enable the gateway &amp; set your credentials on the %sGateway Settings%s page.', 'ppdg' ), '<strong>', '</strong>', '<a href="' . ppdg_settings_tab_url() . '">', '</a>' ); ?>
			</p>
		</div><?php
	}
}
...
function ppdg_paypal_return() {
	...
	$ppdg_params = array(
		...
		'msgWaiting'  => __( "This won't take a minute", 'ppdg' ),
		'msgComplete' => __( 'Payment Processed', 'ppdg' ),
	);

	wp_localize_script( 'ppdg-return', 'ppdg', $ppdg_params );

// Return an intermediary page with a loading image ?>
<html>
<head>
<title><?php __( 'Processing...', 'ppdg' ); ?></title>
...
<div id="right_frame">
	...
	<?php if ( $is_paying ) { // Paid for order ?>
		<?php _e( 'Processing payment', 'ppdg'); ?>
...
setTimeout('if (window!=top) {top.location.replace("<?php echo $location; ?>");}', 1500);
</script>
<?php endif; ?>
</body>
</html>
...
function ppdg_action_links( $links ) {
	...
	$plugin_links = array(
		'<a href="' . ppdg_settings_tab_url() . '">' . __( 'Settings', 'ppdg' ) . '</a>',
		'<a href="http://docs.woothemes.com/document/paypal-digital-goods-for-express-checkout-gateway/">' . __( 'Docs', 'ppdg' ) . '</a>',
		'<a href="http://support.woothemes.com">' . __( 'Support', 'ppdg' ) . '</a>',
	);
