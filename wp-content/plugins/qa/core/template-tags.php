...
function get_the_qa_menu( ) {
	...
	if ( ($user_ID == 0 && qa_visitor_can('publish_questions')) || current_user_can( 'publish_questions' )) {
		$menu[] = array(
		'title' => __( 'Friends', QA_TEXTDOMAIN ),
		'type' => 'ask',
...
/*	$out .= "<div id='qa-menu'>";
	...
	$out .= "</ul>"; 
$out .= "<div class='create-post'>";
$out .= "<span class='icon-plus'></span>";
	$out .= "<a href='http://mariasplace.com/questions/ask/'>Ask Question</a>";
	$out .= "</div>";
...
function get_the_qa_action_links( $id ) {
	...
	foreach ( $links as $type => $title ) {
		if ( 'flag' == $type ) {
			//$flag_link = '<a name="qa_report" href="javascript:void(0)" onClick="javascript:document.getElementById(\'qa_flag_form_'.$id.'\').style.display=\'block\';" >'. $title . '</a><span class="flag-span"></span>';
			if ( isset( $_GET['flag_received'] ) )
			$links[ 'flag' ] = '<span class="reported">' . __('Your report has been received.', QA_TEXTDOMAIN ) . '</span>';
	...
	$out .= '<div class="qa-action-links">';
        $out .= '<a title="Report this item" id="like-activity-<?php get_the_ID(); ?>" class="like" href="#">
...
function the_qa_flag_form( $id ) {
	...
	$f  = '';
	$f .= '<div id="qa_flag_form_'. $id .'" class="qa-flag" >';
...
//		</div>';
	...
	$f .= '<input type="submit" value="'.__('', QA_TEXTDOMAIN).'" />';
	$f .= '</form>';
	$f .= '<br />';
...
function get_the_question_voting( $question_id = 0 ) {
	...
	$out  = '';
	$out .= '<div class="qa-voting-box" id="'.$question_id.'">';
	$out .= $buttons['up'];
  //$out .='<span class="span '.$question_id.'"></span>';
...
function get_the_answer_voting( $answer_id ) {
	...
	$out .=$buttons['up'];
        $out .='<span class="vote-up-off"></span>';
	//$out .= '<span title="' . __( 'Score', QA_TEXTDOMAIN ) . '">' . number_format_i18n( $up - $down ) . '</span>';
	//$out .= $buttons['down'];
...
function get_the_question_status( $question_id = 0 ) {
	...
	$out .=	"<div class='mini-count'><a href='". qa_get_url( 'single', $question_id )."' >" . number_format_i18n( $count ) . "</a></div>";
	$out .=	"<div>" . _n( 'answer', 'Comments', $count, QA_TEXTDOMAIN ) . "</div>";
	$out .= "</div>";
...
function get_the_answer_form() {
 ...
 $out .= get_avatar( $current_user->ID, 64 );
 } 
$out .= '<p><textarea name="answer" class="wp32" placeholder="enter your comment">' .esc_textarea( $answer->post_content ) . '</textarea></p>';
	} else
