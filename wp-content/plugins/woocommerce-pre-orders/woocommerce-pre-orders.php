...
class WC_Pre_Orders {
	...
	/** plugin text domain */
	const TEXT_DOMAIN = 'wc-pre-orders';
	...
	public function init() {
		...
		if ( is_admin() && ! defined( 'DOING_AJAX' ) ) {
			...
			// load admin
			require( 'classes/class-wc-pre-orders-admin.php' );
			$this->admin = new WC_Pre_Orders_Admin();
	...
	public function add_pay_later_gateway( $gateways ) {
		...
		require( 'classes/gateways/class-wc-pre-orders-gateway-pay-later.php' );
	...
	public function pre_order_countdown_shortcode( $atts ) {
		...
		require( 'classes/shortcodes/class-wc-pre-orders-shortcode-countdown.php' );
	...
	public function add_email_classes( $email_classes ) {
		...
		foreach ( array( 'new-pre-order', 'pre-order-available', 'pre-order-cancelled', 'pre-order-date-changed', 'pre-ordered' ) as $class_file_name ) {
			require( "classes/emails/class-wc-pre-orders-email-{$class_file_name}.php" );
		}
	...
	public function add_configure_plugin_link( $actions ) {
		...
		return ( array_merge( array( 'configure' => sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=wc_pre_orders' ), __( 'Configure', self::TEXT_DOMAIN ) ) ),
			$actions )
		);
	}
	...
	/**
	 * Log errors to WooCommerce log if error logging is enabled
	 * /woocommerce/logs/pre-orders.txt
	 *
	 * @since  1.0
