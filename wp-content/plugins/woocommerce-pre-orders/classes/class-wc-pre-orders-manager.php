...
class WC_Pre_Orders_Manager {
	...
	public static function get_all_pre_orders_by_product( $product ) {
		...
		foreach ( $order_ids as $order_id ) {
			$orders[] = new WC_Order( $order_id->id );
		}
	...
	public static function change_release_date_for_all_pre_orders( $product, $new_availability_date, $message = '' ) {
		...
		foreach( $orders as $order ) {
			...
			$order->add_order_note( sprintf( __( 'Pre-Order Release Date Changed to %s', WC_Pre_Orders::TEXT_DOMAIN ), WC_Pre_Orders_Product::get_localized_availability_date( $product, __( 'N/A', WC_Pre_Orders::TEXT_DOMAIN ) ) ) );
	...
	public static function cancel_pre_order( $order, $message = '' ) {
		...
		$order->add_order_note( __( 'Pre-Order Cancelled', WC_Pre_Orders::TEXT_DOMAIN ) );
