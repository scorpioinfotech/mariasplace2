...
class WC_Pre_Orders_Cart {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_before_calculate_totals', array( $this, 'maybe_add_pre_order_fee' ) );
	...
	public function get_item_data( $item_data, $cart_item ) {
		...
		$item_data[] = apply_filters( 'wc_pre_orders_cart_item_meta', array(
			'name'    => $name,
	...
	public function validate_cart( $valid, $product_id ) {
		...
		if ( WC_Pre_Orders_Product::product_can_be_pre_ordered( $product_id ) ) {
			...
			if( $woocommerce->cart->get_cart_contents_count() >= 1 ) {
				...
				$woocommerce->add_message( __( 'Your previous cart was emptied because pre-orders must be purchased separately.', WC_Pre_Orders::TEXT_DOMAIN ) );
			}
			...
			if ( $this->cart_contains_pre_order() ) {
				...
				$woocommerce->add_error( __( 'This product cannot be added to your cart because it already contains a pre-order, which must be purchased separately.', WC_Pre_Orders::TEXT_DOMAIN ) );
	...
	public static function get_pre_order_product() {
		...
		if ( self::cart_contains_pre_order() ) {
			...
			$items = array_values( $woocommerce->cart->cart_contents );
