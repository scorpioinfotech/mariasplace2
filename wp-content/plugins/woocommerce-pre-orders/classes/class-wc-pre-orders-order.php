...
class WC_Pre_Orders_Order  {
	...
	public static function get_pre_order_status_to_display( $order ) {
		...
		switch ( $status ) {
			case 'active' :
				$status_string = __( 'Active', WC_Pre_Orders::TEXT_DOMAIN );
				break;
			case 'completed' :
				$status_string = __( 'Completed', WC_Pre_Orders::TEXT_DOMAIN );
				break;
			case 'cancelled' :
				$status_string = __( 'Cancelled', WC_Pre_Orders::TEXT_DOMAIN );
				break;
			default :
