...
<?php if ( 'pending' == $order->status ) : ?>
	...
	<p><?php printf( __( "Your pre-order is now available, but requires payment. %sPlease pay for your pre-order now.%s", WC_Pre_Orders::TEXT_DOMAIN ), '<a href="' . $order->get_checkout_payment_url() . '">', '</a>' ); ?></p>

<?php elseif ( 'failed' == $order->status || 'on-hold' == $order->status ) : ?>

	<p><?php printf( __( "Your pre-order is now available, but automatic payment failed. %sPlease update your payment information now.%s", WC_Pre_Orders::TEXT_DOMAIN ), '<a href="' . $order->get_checkout_payment_url() . '">', '</a>' ); ?></p>

<?php else : ?>

<p><?php _e( "Your pre-order is now available. Your order details are shown below for your reference.", WC_Pre_Orders::TEXT_DOMAIN ); ?></p>
