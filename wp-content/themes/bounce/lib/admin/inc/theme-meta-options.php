...
extract($args); global $post; ?>
	...
	<div id="meta-box-<?php echo $name; ?>" class="meta-box">
		...
		<?php if($title) { ?><strong><?php echo $title; ?></strong><div class="clear"></div><?php } ?>
		...
		<?php $args = array('post_type' => 'attachment', 'post_parent' => $post->ID, 'numberposts' => -1, 'orderby' => 'date', 'order' => 'desc', 'post__not_in' => array(get_post_thumbnail_id())); $attachments = get_children($args); ?>		
		<?php if($attachments) { foreach ($attachments as $attachment) { ?>
			<?php if($attachment->post_mime_type == 'image/jpeg' OR $attachment->post_mime_type == 'image/jpg' OR $attachment->post_mime_type == 'image/png' OR $attachment->post_mime_type == 'image/gif') { $image = aq_resize(wp_get_attachment_url($attachment->ID), 50, 50, true, true, true); } else { $image = site_url().'/wp-includes/images/crystal/video.png'; } ?>
